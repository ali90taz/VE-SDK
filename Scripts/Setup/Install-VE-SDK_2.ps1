...
$vitaSdkDest = $vitaEngineSdkDest + "\ThirdParty\Toolchains"
...
if ($Global:installFlag) {
    ...
    if (($Global:nodeFound) -and ($Global:gitFound) -and ($Global:codeFound)) {   
        ...
        Invoke-WebRequest -Uri $vitaSdkSrc -OutFile "$($Env:Temp + '/vitasdk.tar.bz2')"
        printText -t " [DONE]" -fc green
        printText -t "  Extracting .tar.bz2 file..." -fc cyan -fs "i" -f "nnl"
        tar.exe -xf "$($Env:Temp + '\vitasdk.tar.bz2')" -C "$vitaSdkDest"
        printText -t " [DONE]" -fc green
